...
function TableView({ data, columns, rowsPerPage, onEdit, onDelete, showCheckboxes = true, showDeleteIcon = true, loading = false, showActionsColumn = true }) {
  ...
  const renderContent = () => {
    // Priorité 1 : Afficher le chargement
    if (loading) {
      return renderLoadingState();
    }

    // Priorité 2 : Afficher "Aucun résultat" si pas de données ET pas en cours de chargement
    if (data && data.length === 0) {
      return renderEmptyState();
    }

    // Priorité 3 : Afficher la table avec les données
    return (
