...
const useDistrictStore = create((set) => ({
  ...
  fetchDistricts: async () => {
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/annuaire/', {
        headers: {
  ...
  updateDistrict: async (id, updatedDistrict) => {
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`http://127.0.0.1:8000/api/annuaire/${id}/`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  deleteDistrict: async (id) => {
    try {
      const token = localStorage.getItem('access_token');
      await fetch(`http://127.0.0.1:8000/api/annuaire/${id}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
